...
__device__ bool placeFeature(FeaturePlacement featurePlacement, ivec3 worldBlockPos, Block* block)
{
    ...
    const vec3 pos = worldBlockPos - featurePos;
    ...
    case Feature::PURPLE_MUSHROOM: 
    {
        ...
        {
            ctrlPts[i] = (endPoint * (i / lastCtrlPtIndex)) + (vec3(u11(rng), u11(rng), u11(rng)) * vec3(6, 2, 6));
        }
        ...
        for (int i = 0; i < SPLINE_SIZE; ++i)
        {
            ...
            {
                pos2 = pos1 + normalize(pos1 - spline[i - 1]) * (0.5f * u01(rng) + 2.5f);
            }
            ...
            if (i < lastSplineIndex)
            {
                ...
                float x = t - 0.5f;
                radius = (4.f * x * x + 1.5f) * 0.8f;
                potentialBlock = Block::SNOW;
            }
            else
            {
                radius = (7.f * u01(rng) + 12.f) * mix(0.8f, 1.2f, (height - 33.f) / 40.f);
                potentialBlock = Block::GRAVEL;
            }
